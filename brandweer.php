<?php
	...
	*/
	...
	?>({
			...
			"items":[
<?php
		//Alle brandweer meldingen ophalen uit de database van de server
			...
			$result = mysql_query("SELECT `meldingen`.*, `projects`.id as project
				...
				WHERE `meldingen`.`type` = 'brandweer'
			...
			//Alle laatste 30 brandweer meldingen zijn opgehaald. Connectie met mysql server kan gesloten worden
			mysql_close($con);
?>] 
})<?php
	if(!isset($_GET["jsoncallback"])){
